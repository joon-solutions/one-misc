...
result_before as (
  select sum(dml_cntr_ltst_mvmt_snap_totald_stay_dys), 'before' as item from before
),
...
result_after as (
  select sum(dml_cntr_ltst_mvmt_snap_totald_stay_dys), 'after' as item from after
)
