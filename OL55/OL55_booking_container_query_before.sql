...
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_BKG_RD_TERM_V  AS dmc_bkg_r_d_term_cntr ON dmc_bkg_r_d_term_cntr.bkg_rd_term_cd=dms_bkg_cntr.RD_TERM_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_NOD_LM_V  AS dmc_yard_trnk_pol ON dmc_yard_trnk_pol.nod_cd= dms_bkg_master.TRNK_POL_NOD_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_NOD_LM_V  AS dmc_yard_trnk_pod ON dmc_yard_trnk_pod.nod_cd= dms_bkg_master.TRNK_POD_NOD_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_CUSTOMER_LM_V  AS dmc_customer_shpr ON dms_bkg_master.SHPR_CUST_CD=dmc_customer_shpr.CUST_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_CUSTOMER_LM_V  AS dmc_customer_antfy ON dms_bkg_master.ANTFY_CUST_CD=dmc_customer_antfy.CUST_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_CUSTOMER_LM_V  AS dmc_customer_cnee ON dms_bkg_master.CNEE_CUST_CD=dmc_customer_cnee.CUST_CD
...
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_CUSTOMER_LM_V  AS dmc_customer_bkg_ctrl ON dms_bkg_master.BKG_CTRL_PTY_CUST_CD=dmc_customer_bkg_ctrl.CUST_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_CUSTOMER_LM_V  AS dmc_customer_ctrt ON dms_bkg_master.SC_CUST_CD=dmc_customer_ctrt.CUST_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_CUSTOMER_LM_V  AS dmc_customer_fwrd ON dms_bkg_master.FWRD_CUST_CD=dmc_customer_fwrd.CUST_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_CUSTOMER_LM_V  AS dmc_customer_ntfy ON dms_bkg_master.NTFY_CUST_CD=dmc_customer_ntfy.CUST_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_CUSTOMER_LM_V  AS dmc_customer_esi_bkg ON dms_bkg_master.BKG_PTY_CUST_CD=dmc_customer_esi_bkg.CUST_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_NOD_LM_V  AS dmc_yard_n1st_pol ON dms_bkg_master.N1ST_POL_NOD_CD=dmc_yard_n1st_pol.nod_cd
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_NOD_LM_V  AS dmc_yard_lst_pod ON dms_bkg_master.POD_NOD_CD=dmc_yard_lst_pod.nod_cd
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_LOCATION_V AS dmc_loc_por ON dms_bkg_master.POR_CD=dmc_loc_por.LOC_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_LOCATION_V AS dmc_loc_del ON dms_bkg_master.DEL_CD=dmc_loc_del.LOC_CD
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_NOD_LM_V  AS dmc_yard_del ON dms_bkg_master.DEL_NOD_CD=dmc_yard_del.nod_cd
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_NOD_LM_V  AS dmc_yard_act_mty_pkup ON dms_bkg_cntr.ACT_MTY_PKUP_YD_CD=dmc_yard_act_mty_pkup.nod_cd
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_NOD_LM_V  AS dmc_yard_act_mty_rtn ON dms_bkg_cntr.ACT_MTY_RTN_YD_CD=dmc_yard_act_mty_rtn.nod_cd
...
LEFT JOIN one-global-dde-prod.DM_VIEWS.DMC_PERIOD_V  AS dmc_period_bkg_rqst ON (DATE(timestamp( dms_bkg_master.XTER_RQST_RQST_DT ) ))=(DATE(PARSE_DATE("%Y%m%d", dmc_period_bkg_rqst.BSE_DT) ))
